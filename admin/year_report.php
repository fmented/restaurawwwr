...
$q = sprintf($x, $dt . "-01-01", $dt . "-12-31");
require './months.php';


function get_transaction($qr, $x, $m)
{
    $str = '';
    $db = Database::connect();
    ...
    while ($item = $statement->fetch()) {
        ...
        $str .= ' <tr>
                    ...
                    <td><a href="date_report.php?date=' . $x . '-' . $d[2] . '">' . $d[2] . '</a> <a href="month_report.php?date=' . $x . '-' . $d[1] . '">' . $m[$d[1]] . '</a></td>
                    <td>' . $item['time'] . '</td>
                    <td>' . $item['total'] . '</td>
                    <td>' . $item['cash'] . '</td>
                    <td>' . $item['cc'] . '</td>
...
<body>
    ...
    <div class="container admin">
        <div class="row">
            <h1><strong><?php echo $dt; ?> Reports</strong></h1>
            <br>
            <div class="table-responsive">
                <?php echo get_transaction($q, $dt, $months); ?>
                <br>
            </div>
            <!-- <?php echo $q;
                    echo $_GET['date']; ?> -->
